...
const countWhich = function (list, cb) {
  const result = []; //Elements respecting the conditions of the callback
  if (!Array.isArray(list)) return false; //If input not an array, return false
  for (let element of list) {
    //Any element following the conditions is stored in result
    if (cb(element)) {
      result.push(element);
    }
  }

  return result.length; //Number of elements satisfying the callback conditions
};
